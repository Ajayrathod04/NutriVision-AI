...
import { 
  LayoutDashboard, Activity, Brain, Scan, ShoppingBag, Trophy, ListTodo, LogOut, Menu, X, Utensils, Calculator, Heart
} from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <>
      <button 
        className="lg:hidden fixed top-4 right-4 z-50 p-2 bg-dark border border-white/10 rounded-lg text-white"
      ...
      `}>
        <div className="flex flex-col h-full p-6">
          <div className="flex items-center gap-3 mb-10 px-2">
            ...
            <h1 className="text-xl font-extrabold tracking-tight text-white">NutriMind <span className="text-accent">AI</span></h1>
          </div>

          <div className="flex-1 overflow-y-auto space-y-8 scrollbar-hide">
            {sections.map((section, idx) => (
              <div key={idx}>
                <h3 className="text-xs font-bold text-gray-500 tracking-widest mb-4 px-2 uppercase">{section.title}</h3>
